...
Alle Parameter werden über **typsichere Python Dataclasses** verwaltet, keine Plain Dicts oder Magic Strings.
...
| Typsichere Parameter für Geometrie, Rendering, Kamera-Effekte
...
* **Validation**: Fehler zur Compile-Zeit statt Runtime
...
**Strategie**: Jede Pipeline-Stufe erzeugt Debug-PNG in `app/static/output/`
...
| **Segmentation**
| `pieces_<filename>_<id>.png` (RGBA mit transparentem Background)

| **Edge Detection**
| `edges_<filename>_<id>.png` (Konturen + Ecken + Labels)

| **Matching**
| `matches_<filename>_<id>.png` (Side-by-Side Edge-Pairs mit Scores)
...
**Generator** nutzt Seeds für deterministisches Verhalten:
...
* Exakte Reproduzierbarkeit von Test-Puzzles
* Bug-Reports mit Seed → 100% nachvollziehbar
